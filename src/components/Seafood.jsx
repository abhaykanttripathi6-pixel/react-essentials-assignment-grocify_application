...
import Meat_Seafood from '../assets/seafood-banner.jpg'
import { FaHeart } from "react-icons/fa";
import Cheese from '../assets/ricotta-cheese.png';

const Seafood = ({ products, darkMode }) => {
    return (
        <section>
            <div className={`${darkMode ? 'bg-black text-white' : 'bg-white text-black'}`}>
                <div className='py-18 flex flex-col gap-20 sm:py-10 md:py-15 xl:py-20'>
                    <div className='h-60 w-full flex justify-center items-center bg-cover bg-no-repeat bg-center sm:h-100'
                        style={{ backgroundImage: `url(${Meat_Seafood})` }}>
                        <h1 className='p-3 bg-white text-xl text-black font-bold rounded-lg md:text-2xl lg:text-4xl'>Meat & Seafood</h1>
                    </div>

                    <div className='p-3 grid grid-cols-[repeat(2,minmax(100px,1fr))] justify-content-center gap-5 sm:p-7 sm:grid-cols-[repeat(auto-fit,minmax(200px,1fr))] sm:gap-10 md:p-15 md:grid-cols-[repeat(auto-fit,minmax(250px,1fr))] md:gap-8 xl:p-15 lg:grid-cols-[repeat(auto-fit,minmax(300px,1fr))] lg:gap-15'>
                        {
                            products.filter(product => product.category === 'Seafood').map(item => {
