...
import All_Banner from '../assets/all-banner.jpg'
import { FaHeart } from "react-icons/fa";

const AllProducts = ({ products, darkMode }) => {
    return (
        <section>
            <div className={`${darkMode ? 'bg-black text-white' : 'bg-white text-black'}`}>
                <div className='py-18 flex flex-col gap-20 sm:py-10 md:py-15 xl:py-20'>
                    <div className='h-60 w-full flex justify-center items-center bg-cover bg-no-repeat bg-center sm:h-100'
                        style={{ backgroundImage: `url(${All_Banner})` }}>
                        <h1 className='p-3 bg-white text-xl text-black font-bold rounded-lg md:text-2xl lg:text-4xl'>All Products</h1>
                    </div>
                                ...
                                return <div key={id} className='p-4 bg-gray-200 rounded-xl md:p-8 hover:shadow-lg hover:shadow-gray-400 hover:-translate-y-1 transition-all transition-duration-300'>
                                    ...
                                    <div>
                                        <figure>
                                            ...
                                            <figcaption className='text-lg font-bold text-center text-black'>{name}</figcaption>
                                        </figure>
                                    </div>
                                    <div className='flex flex-col items-center gap-2'>
                                        <p className='text-lg font-bold text-black'>{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(price)}</p>
